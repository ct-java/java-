...
public class JedisUtil {
	...
	public final static JedisUtil getInstance(JedisPool jedisPool) {
		if (jedisPool == null) {
			throw new NullPointerException("参数'jedisPool'不能为null");
		}
	...
	public JedisPool getJedisPool() {
		if (jedisPool == null)
			throw new NullPointerException("'jedisPool'不存在");
		return jedisPool;
	}
	...
	/*
	 * sinterstore 这个命令类似于 SINTER 命令，但它将结果保存到 destination 集合，而不是简单地返回结果集。 如果
	 * destination 集合已经存在，则将其覆盖。destination 可以是 key 本身
	 * 
	 * @return 新集合的记录数
	 */
	...
	/*
	 * smove 将成员从源集合移除放入目标集合 </br> 如果源集合不存在或不包含指定成员，不进行任何操作，返回0</br>
	...
	/*
	 * 从集合中删除指定成员 移除集合 key 中的一个或多个 member 元素，不存在的 member 元素会被忽略。当 key 不是集合类型，返回一个错误。
	 * 
	...
	/*
	 * zrem 移除有序集 key 中的一个或多个成员，不存在的成员将被忽略。当 key 存在但不是有序集类型时，返回一个错误。在 Redis 2.4
	 * 版本以前， ZREM 每次只能删除一个元素。
	 * 
	...
	public static void main(String[] args) {
		JedisUtil js = JedisUtil.getInstance(new JedisPool("192.168.18.130",6379));
